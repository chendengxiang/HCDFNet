...
def beam_search(self, memory, beam_size=5, max_len=52):
        batch_size = memory.size(1)
        
        # Initialize the beams: each element is (sequence, score)
        beams = [[list(), 0.0] for _ in range(batch_size)]  # (sequence, score)
        finished_sequences = []

        for i in range(max_len):
            all_candidates = []

            # Iterate over each beam (one per batch)
            for beam in beams:
                seq, score = beam
                inputs = torch.tensor(seq).unsqueeze(0).to(device)

                # Get the model prediction for the next token
                scores, _, _, _ = self.forward(memory, inputs, [len(inputs)])
                topk_scores, topk_indices = torch.topk(scores[0, -1], beam_size)

                # Add the candidates to all_candidates
                for j in range(beam_size):
                    candidate = [seq + [topk_indices[j].item()], score + topk_scores[j].item()]
                    all_candidates.append(candidate)

            # Select the top `beam_size` candidates from all_candidates
            ordered = sorted(all_candidates, key=lambda x: x[1], reverse=True)
            beams = ordered[:beam_size]

            # If any sequence has finished (i.e., <end> token is reached), add to finished_sequences
            for seq, score in beams:
                if seq[-1] == self.vocab_size - 1:  # Assuming <end> token is at vocab_size-1
    ...
    with torch.no_grad():
        for i, (image_pairs, caps, caplens, allcaps) in enumerate(
            ...
            while True:
                ...
                if beam_search:
                    return self.beam_search(encoder_out, beam_size)
                encoder_out = encoder_out.permute(1, 0, 2,3)
