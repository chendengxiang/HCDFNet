...
def train(args, train_loader, encoder_image,encoder_feat, decoder, criterion, encoder_image_optimizer,encoder_image_lr_scheduler,encoder_feat_optimizer,encoder_feat_lr_scheduler, decoder_optimizer, decoder_lr_scheduler, epoch):
    ...
    for i, (img_pairs, caps, caplens) in enumerate(train_loader):
        ...
        fused_feat = encoder_feat(imgs_A,imgs_B).to(device) # fused_feat: (S, batch, feature_dim)
        scores, caps_sorted, decode_lengths, sort_ind,gt_caption_embed = decoder(fused_feat, caps, caplens)
        ...
        # 对比损失（例如余弦相似度 + InfoNCE）
        # 假设 gt_caption_embed: (B, L, D)，取 [CLS] 或平均向量作为整体语义表示
        caption_repr = gt_caption_embed.mean(dim=1)  # (B, D)
        image_repr = fused_feat.mean(dim=1)              # (B, D)
        # 归一化后计算对比损失（可自定义，也可以使用现成的 NT-Xent）
        loss_clip = contrastive_loss(caption_repr, image_repr)  # 你自己的实现
        # 总损失
        loss = loss_ce + 0.5 * loss_clip  # alpha 是权重，例如 0.5
...
def main(args):
    ...
    # Initialize
    # Encoder
    print("当前args.encoder_image是",args.encoder_image)
